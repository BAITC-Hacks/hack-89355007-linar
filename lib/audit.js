...
export function renderAuditReport(audit,decisions){
 ...
 for(const [i,f] of confirmed.entries()){
  ...
  if(f.requirements.length)lines.push('Нормативное основание: '+f.requirements.map(r=>`${r.sourceTitle}, пункт ${r.clause} (${r.sourceKind==='external'?'справочный источник':'учебный внутренний источник'})${r.sourceUrl?`, ${r.sourceUrl}`:''}`).join('; '));
