...
export function analyze(documents) {
 const functions=[],units=[],mappings=[],warnings=[];
 for(const doc of documents) {
  let unit=null;
  for(const [index,raw] of doc.text.split(/\r?\n/).entries()) {
   const line=raw.trim(); if(!line)continue;
   const mapping=line.match(/^Реорганизация:\s*(.+?)\s*(?:→|->)\s*(.+)$/i);
   if(mapping&&doc.period==='after')mappings.push({before:mapping[1],after:mapping[2],source:{documentId:doc.id,document:doc.name,point:`строка ${index+1}`,quote:line}});
   const head=line.match(/^(?:Подразделение|Департамент|Отдел)\s*:\s*(.+)$/i);
   if(head) {unit=head[1];units.push({name:unit,period:doc.period,source:{documentId:doc.id,document:doc.name,point:`строка ${index+1}`,quote:line}});continue;}
   const f=line.match(/^(\d+(?:\.\d+)*)(?:[.)]|\s)\s*(.{12,})$/);
   if(f&&unit)functions.push({id:`${doc.id}-${index}`,unit,period:doc.period,text:f[2],source:{documentId:doc.id,document:doc.name,point:f[1],quote:line}});
  }
 ...
 return {documents,functions,table,structure,findings:findings.map((f,i)=>({...f,id:`R-${String(i+1).padStart(3,'0')}`})),warnings,createdAt:new Date().toISOString(),method:'Локальное сопоставление значимых слов и правил разделения обязанностей. Оценки не являются вероятностями.'};
}
