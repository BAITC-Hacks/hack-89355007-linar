...
export function analyze(documents) {
 ...
 for(const doc of documents) {
  ...
  for(const [index,raw] of doc.text.split(/\r?\n/).entries()) {
   ...
   if(head) {unit=head[1];headingCount++;units.push({name:unit,period:doc.period,source:{documentId:doc.id,document:doc.name,point:`строка ${index+1}`,quote:line}});continue;}
   const f=line.match(/^(\d+(?:\.\d+)*)(?:[.)]|\s)\s*(.{12,})$/);
   if(f&&unit){functionCount++;functions.push({id:`${doc.id}-${index}`,unit,period:doc.period,text:f[2],source:{documentId:doc.id,document:doc.name,point:f[1],quote:line}});}
