...
test('Настоящие учебные DOCX и XLSX проходят загрузчик и выявляют ожидаемые отклонения',async()=>{
 ...
 const child=spawn(process.execPath,['server.js'],{cwd:new URL('../',import.meta.url),env:{...process.env,PORT:String(port),OPENAI_API_KEY:''},stdio:'ignore'});
 try{
  let response;
  ...
  assert.ok(response,'Сервер запустился');
